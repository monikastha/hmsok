    ...
    void deleteRoom() {
        ...
        if (selectedRow != -1) {
            ...
            try {
                ...
                // 1. Get student IDs associated with this room
                PreparedStatement pstSelectStudentIds = con.prepareStatement("SELECT std_id FROM student WHERE room_id = ?");
                pstSelectStudentIds.setInt(1, roomId);
                ResultSet rs = pstSelectStudentIds.executeQuery();

                // Collect student IDs to delete their fees
                java.util.List<Integer> studentIdsToDelete = new java.util.ArrayList<>();
                while (rs.next()) {
                    studentIdsToDelete.add(rs.getInt("std_id"));
                }
                rs.close();
                pstSelectStudentIds.close();

                // 2. Delete associated fee records (if fee table is linked to student_id)
                // This assumes your 'fee' table has a 'std_id' foreign key.
                if (!studentIdsToDelete.isEmpty()) {
                    // Construct a dynamic IN clause for multiple student IDs
                    String inClause = String.join(",", java.util.Collections.nCopies(studentIdsToDelete.size(), "?"));
                    PreparedStatement pstDeleteFees = con.prepareStatement("DELETE FROM fee WHERE std_id IN (" + inClause + ")");
                    for (int i = 0; i < studentIdsToDelete.size(); i++) {
                        pstDeleteFees.setInt(i + 1, studentIdsToDelete.get(i));
                    }
                    pstDeleteFees.executeUpdate();
                    pstDeleteFees.close();
                }

                // 3. Delete associated student records
                PreparedStatement pstDeleteStudents = con.prepareStatement("DELETE FROM student WHERE room_id = ?");
                pstDeleteStudents.setInt(1, roomId);
                pstDeleteStudents.executeUpdate();
                pstDeleteStudents.close();

                // 4. Delete the room itself
                PreparedStatement pstDeleteRoom = con.prepareStatement("DELETE FROM room WHERE room_id = ?");
                pstDeleteRoom.setInt(1, roomId);
                pstDeleteRoom.executeUpdate();
                pstDeleteRoom.close();

                con.commit(); // Commit transaction if all successful
                JOptionPane.showMessageDialog(this, "Room deleted successfully!");
                viewRoom(); // Refresh the table
            ...
            JOptionPane.showMessageDialog(this, "Please select a room to delete!", "Selection Required", JOptionPane.INFORMATION_MESSAGE);
        }
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Admin Room Manager");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.add(new AdminRoomManager());
            frame.pack();
